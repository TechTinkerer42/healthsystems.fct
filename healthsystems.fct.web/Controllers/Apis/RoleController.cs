    ...
    public class RoleController : ApiController
    {
        public HttpResponseMessage Get()
        {
            using (var session = NHibernateHelper.CreateSessionFactory())
            {
                ...
                {
                    // collect objects
                    var data = new List<Role>(session.CreateCriteria(typeof(Role)).List<Role>());

                    data.ForEach(x =>
                    {
                        if (x.Users != null) x.Users = null;
                    });
        ...
        public HttpResponseMessage Get(int id)
        {
            using (var session = NHibernateHelper.CreateSessionFactory())
            {
                using (var transaction = session.BeginTransaction())
                {
                    ...
                    if (entity != null)
                    {
                        if (entity.Users != null) entity.Users = null;
                    }
        ...
        public HttpResponseMessage Post(Role entity)
        {
            using (var session = NHibernateHelper.CreateSessionFactory())
            {
                using (var transaction = session.BeginTransaction())
